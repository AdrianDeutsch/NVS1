...
function zeigeFrage() {
    if (aktuelleFrage < fragen.length) {
        ...
        const frageObjekt = fragen[aktuelleFrage];
        const frage = document.createElement('h2');
        frage.textContent = frageObjekt.frage;
        quizContainer.appendChild(frage);
...
function zeigeErgebnis() {
    const quizContainer = document.getElementById('quiz-container');
    quizContainer.innerHTML = `Du hast ${punkte} von ${fragen.length} möglichen Punkten erreicht.`;
}
